...
def ensure_di1_batch(env, dry_run):
    ...
    if dry_run:
        ...
        # A sentinel, so the plan below reads as "these 53 move there"
        # instead of "these 53 are stranded" - which is what it printed the
        # first time and was simply untrue.
        return values["name"], True
    return env["dhb.batch"].create(values), True


def main(env):
    # Checked as dates, not as "not empty". The first run of this script was
    # given YYYY-MM-DD as a placeholder, which is a perfectly truthy string
    ...
    for label, value in (("IGC_START", IGC_START), ("IGC_END", IGC_END),
        ...
        try:
            ...
            raise SystemExit(
                "%s is %r, which is not a date. Write it as 2026-08-22."
                % (label, value))
    if IGC_END < IGC_START or DI1_END < DI1_START:
        raise SystemExit("A batch cannot end before it starts.")
    ...
    for label, emails, target in plan:
        if isinstance(target, str):          # the dry-run sentinel
            shown = target + "   (would be created)"
        elif target:
            shown = target.display_name
        else:
            shown = "*** NO BATCH - left where they are ***"
        print("\n--- %s -> %s" % (label, shown))
